...
public class LgDispatcherServlet extends HttpServlet {
    ...
    public void init(ServletConfig config) {
        ...
        // 5 构造一个HandlerMapping处理器映射器，将配置好的url和Method建立映射关系,同时解析Secur信息
        initHandlerMappingAndSecurity();
...
//    }
    ...
    /*
        ...
        目的：将url和method建立关联
     */
    private void initHandlerMappingAndSecurity() {
        if(ioc.isEmpty()) {return;}
        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                if(method.isAnnotationPresent(Security.class)){
                    users=method.getAnnotation(Security.class).value();
                }
                /**
                 * Object controller, Method method, Pattern pattern,Pattern basePattern,
                 *                    String[] baseUsers,String[] users
                 */
                Handler handler = new Handler(entry.getValue(),method, Pattern.compile(url),baseUser,users);
...
//        String baseUrl=handler.getBasePattern().pattern();
//        String url=handler.getPattern().pattern();
        String[] baseUser=handler.getBaseUsers();
        String[] user=handler.getUsers();
        String reqUser=req.getParameter("name");
        req.setCharacterEncoding("GBK");
        resp.setCharacterEncoding("GBK");
        if(null==reqUser||"".equals(reqUser.trim())){

            resp.getWriter().write("必须有name才能访问");
            return;
        }
        //如果访问用户不包含在controller上的
        if(baseUser!=null&&!Arrays.asList(baseUser).contains(reqUser)){
            resp.getWriter().write("该用户无法访问，无权限");
            return;
        }
        if(user!=null&&!Arrays.asList(user).contains(reqUser)){
            resp.getWriter().write("该用户无法访问，无权限");
            return;
        }
        // 参数绑定
